...
bool CUdpServer::Start(unsigned long IP, unsigned short Port)
{
	...
	m_Io.set(m_Loop);
	m_Io.set<CUdpServer, &CUdpServer::__ReadCallback>(this);
...
void CUdpServer::__ReadCallback(ev::io &watcher, int revents)
{
	if (EV_ERROR & revents) 
	{
		return;
